# DocHub HTTP API（Hono）

> 与 MCP 共用同一个 Hono 应用、同一端口（默认 **8276**）。  
...
**注意：** `mcp.enabled=false` 时 **不启动** `@hono/node-server`，上述路由均不可达；UI 应通过 **IPC** 读取状态（`mcp:getStatus`）。
...
```
main/services/mcp/
├── server.ts       # 创建 Hono app，挂载路由
├── routes/
│   ├── mcp.ts      # app.all('/mcp', ...)
│   └── health.ts   # app.get('/health', ...)
├── tools/          # MCP tool  handlers
└── lifecycle.ts    # serve / close
```
...
export function createApp(deps: AppDeps) {
  const app = createMcpHonoApp() // 或 new Hono()，按 SDK 推荐方式
  ...
  app.get('/health', (c) => {
    return c.json({
      ...
      mcp: {
        ...
        endpoint: `http://${deps.config.host}:${deps.config.port}/mcp`,
      },
      dataDir: deps.config.dataDir,
      sources: deps.getSourceStats(),
    })
  })

  app.all('/mcp', (c) => deps.transport.handleRequest(c.req.raw))
...
同一 Hono app 可追加 **内部 REST**，前缀建议 `/api/v1/`：
...
- [project-structure.md](./project-structure.md) — 目录与 Hono 选型
