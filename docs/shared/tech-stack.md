...
| MCP            | **官方 `@modelcontextprotocol/sdk`**        | v1       | Streamable HTTP，`/mcp` + `GET /health`，同端口 8276 |
| MCP 运行时     | Node `http`                                 | v1       | Main Process 内绑定 `127.0.0.1:{port}`               |
...
| 正文提取       | @mozilla/readability + jsdom                | v1       | 噪声页面叠加 Readability 再送 mdream                 |
...
| SPA Render | **Playwright**                   | v2；独立 Chromium，Browser/Page 池复用 |
...
| 噪声页面 | **@mozilla/readability** + jsdom → mdream | 已接入预处理    |
...
| Playwright 体积大                | postinstall 下载 Chromium，打包时通过 asarUnpack 带上    |
